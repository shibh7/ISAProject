...
app.use(cors());
app.options('https://amberma2.com', cors());
...
app.post(endPointRoot + "signup/", (req, res) => {
    ...
    let apicheck = `SELECT * FROM apikeys WHERE domain = "${origin}"`;
    
    db.promise(apicheck)
    .then((result0) => {
        if(result0.length == 0){
            res.status(401).end("Unauthorized Access");
        }else if(result0[0].apikey != req.header('api-key')){
            res.status(401).end("Unauthorized Access");
        }else{
            let body = '';
            console.log(req)
...
//Add a Todo item
app.post(endPointRoot + "usertodoitem/:id", (req, res) => {
    ...
    .then((result0) => {
        if(result0.length == 0){
            ...
            req.on('end', function(){
                let jsonObj = JSON.parse(body);
                console.log(jsonObj)
                if( (jsonObj.todoID == null) ||  (jsonObj.todoID == "") ){
                    res.status(401).send("Bad request");
                }else if( (jsonObj.description == null) ||  (jsonObj.description == "") ){
                    res.status(401).send("Bad request");
                }else{
                    let check = `SELECT * FROM todolist WHERE todoID = ${jsonObj.todoID}`;
                    db.promise(check)
                    .then((result) => {
                        if(result.length == 0){
                            res.status(401).send("Bad request");
                        }else{
                            let sql2 = `INSERT INTO todoitems VALUES (NULL, ${jsonObj.todoID}, "${jsonObj.description}")`;
                            return db.promise(sql2)
                        }
                    }).then((result) => {
                        console.log(result);
